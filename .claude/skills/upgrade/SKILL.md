...
- `Goals/**` — Your career goal cascade (`0. Three Year Vision (Career).md`, `1. Yearly Goals (YYYY - *).md`, `2. Monthly Goals (Current).md`)
- `Projects/**` — Your project folders and their CLAUDE.md files (including Decisions & Learnings)
- `Archives/**` — Archived content and archived templates
...
- For `CLAUDE.md`: merge carefully — preserve user customizations (Current Focus, primary/secondary project list) while updating system sections (Skills table, cascade diagram, version number). Do not reintroduce life-area sections.
- For `Templates/`: only update the current career templates (`Daily Note - Career.md`, `Weekly Project Review.md`). Leave anything under `Archives/Templates/` alone.
